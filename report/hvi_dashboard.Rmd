...
spark_lines <- function(x, y, nl_min, nl_max, type = "line") {
    spk_chr(
        ...
        normalRangeMin = nl_min,
        normalRangeMax = nl_max
    )
}
```
...
df_spark <- data_labs %>%
    ...
    summarize_at("lab.result", first, order_by = "lab.datetime") %>%
    mutate(lab.drawn = difftime(lab.datetime, update_time, units = "hours")) %>%
...
spk_hgb <- df_spark %>%
    ...
    filter(n > 1) %>%
    summarize(hgb = spark_lines(lab.drawn, lab.result, 8.5, 10)) %>%
    select(-lab)

spk_platelet <- df_spark %>%
    filter(lab == "Platelet") %>%
    add_count(millennium.id) %>%
    filter(n > 1) %>%
    summarize(platelet = spark_lines(lab.drawn, lab.result, 133, 250)) %>%
    select(-lab)
...
spk_scr <- df_spark %>%
    ...
    add_count(millennium.id) %>%
    filter(n > 1) %>%
    summarize(scr = spark_lines(lab.drawn, lab.result, 0.5, 1.4)) %>%
    select(-lab)
...
spk_wbc <- df_spark %>%
    ...
    add_count(millennium.id) %>%
    filter(n > 1) %>%
    summarize(wbc = spark_lines(lab.drawn, lab.result, 3.7, 10.4)) %>%
    select(-lab)
...
tmp_uop <- data_uop %>%
    ...
    mutate(uop.hr = floor_date(uop.datetime, unit = "hour"),
           uop.date = if_else(hour(uop.hr) <= 7, 
...
df_vitals <- data_vitals %>%
    ...
    select(-vital.units) %>%
    mutate(vital.drawn = difftime(vital.datetime, update_time, units = "hours")) %>%
...
spk_sbp <- df_vitals %>%
    ...
    add_count(millennium.id) %>%
    filter(n > 1) %>%
    summarize(sbp = spark_lines(vital.drawn, vital.result, 90, 140)) %>%
    select(-vital)
...
spk_hr <- df_vitals %>%
    ...
    add_count(millennium.id) %>%
    filter(n > 1) %>%
    summarize(hr = spark_lines(vital.drawn, vital.result, 60, 100)) %>%
    select(-vital)
...
spk_temps <- tmp_temps %>%
    ...
    group_by(millennium.id) %>%
    arrange(vital.datetime, .by_group = TRUE) %>%
    summarize(spk.temp = spark_lines(vital.drawn, vital.result, 97.5, 100.5)) 
```
...
df <- data_patients %>%
    ...
    left_join(spk_scr, by = "millennium.id") %>%
    left_join(spk_wbc, by = "millennium.id") %>%
    left_join(spk_hgb, by = "millennium.id") %>%
    left_join(spk_platelet, by = "millennium.id") %>%
    left_join(spk_sbp, by = "millennium.id") %>%
    left_join(spk_hr, by = "millennium.id") %>%
...
```{r}
col_names <- c("Patient", "Age", "Sex", "LOS", "SOFA", "Temp", "Tmax", "SCr", 
               "WBC", "Hgb", "Plt", "SBP", "HR", "Gluc", "SSI", "UOP", "Avg", 
               "Drips", "Alerts", "PCA", "Doses")
