...
df_pca <- data_measures %>%
    mutate_at("measure.result", as.numeric) %>%
    filter(measure %in% c("PCA Doses Delivered", "PCA Total Demands"),
           measure.datetime >= ymd(today()) - days(1)) %>%
    group_by(millennium.id, measure) %>%
    summarize_at("measure.result", sum, na.rm = TRUE) %>%
    mutate_at("measure", str_replace_all, pattern = c("PCA Total Demands" = "demand",
...
df_uop_hourly <- tmp_uop %>%
    filter(uop_date == today() - days(1)) %>%
    mutate(uop.drawn = difftime(uop.datetime, now(), units = "hours"),
           uop.value = "uop") %>%
    group_by(millennium.id, uop.value) %>%
    arrange(uop.datetime, .by_group = TRUE) %>%
    unite(x_y, uop.drawn, uop.result, sep = ":") %>%
    summarize_at("x_y", funs(paste(., collapse = ","))) %>%
    spread(uop.value, x_y) 
...
df_drips <- data_meds %>%
    filter(med.datetime >= ymd(today()) - days(1),
    ...
    mutate(rate = if_else(med == "Insulin regular", sum, last),
           med_abbrv = str_extract(med, "^.{3}")) %>%
    mutate_at("med_abbrv", str_to_lower) %>%
    # mutate_at("rate.units", str_replace_all, pattern = "microgram", replacement = "mcg") %>%
...
df_gluc <- data_labs %>%
    filter(floor_date(lab.datetime, unit = "day") >= ymd(today()) - days(1),
           lab %in% c("Glucose POC")) %>%
    mutate(lab.drawn = difftime(lab.datetime, now(), units = "hours")) %>%
    group_by(millennium.id, lab) %>%
    arrange(lab.datetime, .by_group = TRUE) %>%
    unite(x_y, lab.drawn, lab.result, sep = ":") %>%
...
df_temps_hourly <- tmp_temps %>%
    mutate(vital.drawn = difftime(vital.datetime, now(), units = "hours"),
           vital.value = "temp") %>%
    group_by(millennium.id, vital.value) %>%
    arrange(vital.datetime, .by_group = TRUE) %>%
    unite(x_y, vital.drawn, vital.result, sep = ":") %>%
    summarize_at("x_y", funs(paste(., collapse = ","))) %>%
    spread(vital.value, x_y) 
...
colDefs1 <- list(
    ...
    list(targets = i + 1, width = '55px', render = JS("function(data, type, full){ return '<span class=spark_temp>' + data + '</span>' }")),
    list(targets = i + 2, width = '55px', render = JS("function(data, type, full){ return '<span class=spark_scr>' + data + '</span>' }")),
    list(targets = i + 3, width = '55px', render = JS("function(data, type, full){ return '<span class=spark_hgb>' + data + '</span>' }")),
    list(targets = i + 4, width = '55px', render = JS("function(data, type, full){ return '<span class=spark_plt>' + data + '</span>' }")),
    list(targets = i + 5, width = '55px', render = JS("function(data, type, full){ return '<span class=spark_wbc>' + data + '</span>' }")),
    list(targets = i + 6, width = '55px', render = JS("function(data, type, full){ return '<span class=spark_sbp>' + data + '</span>' }")),
    list(targets = i + 7, width = '55px', render = JS("function(data, type, full){ return '<span class=spark_hr>' + data + '</span>' }")),
    list(targets = i + 8, width = '55px', render = JS("function(data, type, full){ return '<span class=spark_gluc>' + data + '</span>' }")),
    list(targets = i + 11, width = '55px', render = JS("function(data, type, full){ return '<span class=spark_uop>' + data + '</span>' }"))
    # list(targets = c(i + 8, i + 9), width = '15px')
...
normal_sbp <- c(90, 140)
normal_hr <- c(60, 100)
normal_gluc <- c(80, 180)
normal_uop <- c(50, 150)
normal_temp <- c(97.5, 100.5)

cb_line <- JS(paste0("function (oSettings, json) { $('.spark_scr:not(:has(canvas))').sparkline('html', { ", 
    line_string, ", normalRangeMin: ", normal_scr[1], ", normalRangeMax: ", normal_scr[2], " });\n $('.spark_gluc:not(:has(canvas))').sparkline('html', { ", 
    line_string, ", normalRangeMin: ", normal_gluc[1], ", normalRangeMax: ", normal_gluc[2], " });\n $('.spark_temp:not(:has(canvas))').sparkline('html', { ", 
    line_string, ", normalRangeMin: ", normal_temp[1], ", normalRangeMax: ", normal_temp[2], " });\n $('.spark_uop:not(:has(canvas))').sparkline('html', { ", 
    line_string, ", normalRangeMin: ", normal_uop[1], ", normalRangeMax: ", normal_uop[2], " });\n $('.spark_hgb:not(:has(canvas))').sparkline('html', { ", 
...
caption <- paste0("Updated: ", format(update_time, "%B %d, %Y at %I:%M %p"))

col_names <- c("Patient", "Age", "Sex", "LOS", "SOFA", "Tmax", "Temp", "SCr", "Hgb", "Plt", "WBC", "SBP", "HR", "Gluc", "SSI", "UOP", "Hourly", "Drips", "Alerts", "PCA", "Doses")
```
