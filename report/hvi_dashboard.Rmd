...
df_drips <- data_meds %>%
    ...
    arrange(desc(med.datetime), .by_group = TRUE) %>%
    distinct(millennium.id, med, .keep_all = TRUE) %>%
    filter(rate > 0) %>%
    mutate(med_abbrv = str_extract(med, "^.{3}")) %>%
...
d1 <- df %>%
    ...
    datatable(caption = paste0("Updated: ", format(update_time, "%B %d, %Y at %I:%M %p")),
              rownames = FALSE, 
              colnames = c("Patient", "Age", "Sex", "LOS", "SOFA", "Tmax", "SCr", "Hgb", "Plt", "WBC", "SBP", "HR", "Gluc", "UOP", "Drips", "Alerts"),
