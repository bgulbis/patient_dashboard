---
title: "CVICU Patient Dashboard"
knit: (function(inputFile, encoding) { rmarkdown::render(inputFile, encoding = encoding, output_dir = 'U:', output_file = 'dashboard_cvicu.html') })
output: flexdashboard::flex_dashboard
---

<style>
    body .main-container {
        max-width: 100%;
    }
</style>

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE, message = FALSE, warning = FALSE)
library(tidyverse)
library(readxl)
library(lubridate)
library(DT)
library(sparkline)

spark_lines <- function(x, y, nl_min, nl_max, type = "line") {
    spk_chr(
        values = y,
        type = type,
        xvalues = x,
        width = 50,
        lineColor = "black",
        fillColor = FALSE,
        highlightLineColor = "orange",
        highlightSpotColor = "blue",
        normalRangeMin = nl_min[1],
        normalRangeMax = nl_max[1]
    )
}

ref_normals <- tibble(
    event = c(
        "scr",
        "wbc",
        "hgb",
        "platelet",
        "glucose",
        "hr",
        "sbp",
        "uop",
        "temperature",
        "inr"
    ),
    normal_min = c(0.5, 3.7, 8.5, 133, 80, 60, 90, 50, 97.5, 1.5),
    normal_max = c(1.4, 10.4, 10, 250, 180, 100, 140, 150, 100.5, 3)
)

```

```{r read_data}
file_nm <- "U:/Data/patient_dashboard/dashboard_data_daily.xlsx"

data_dash <- read_excel(file_nm, sheet="dashboard") %>%
    rename_all(tolower)

data_spark <- read_excel(file_nm, sheet="sparklines") %>%
    rename_all(tolower)

data_drips <- read_excel(file_nm, sheet="drips") %>%
    rename_all(tolower)

```

```{r sparklines}
sparks <- data_spark %>%
    mutate(
        hrs = difftime(
            event_datetime,
            now(),
            units = "hours"
        )
    ) %>%
    left_join(ref_normals, by = "event") %>%
    add_count(encntr_id, event) %>%
    filter(n > 1) %>%
    group_by(encntr_id, event) %>%
    summarize_at(
        "result_val", 
        list(
            ~spark_lines(
                x = hrs, 
                y = ., 
                nl_min = normal_min,
                nl_max = normal_max
            )
        )
    ) %>%
    # pivot_wider(
    #     names_from = event, 
    #     values_from = result_val,
    #     names_prefix = "spark_"
    # )
    spread(event, result_val)
```

```{r current_values}
curr_vals <- data_spark %>%
    filter(
        event %in% c(
            "temperature",
            "scr",
            "wbc",
            "hgb",
            "platelet",
            "inr"
        )
    ) %>%
    arrange(encntr_id, event, desc(event_datetime), desc(event_id)) %>%
    distinct(encntr_id, event, .keep_all = TRUE) %>%
    select(encntr_id, event, result_val) %>%
    pivot_wider(
        names_from = event, 
        values_from = result_val,
        names_prefix = "curr_"
    )
```

```{r max_values}
max_vals <- data_spark %>%
    filter(event == "temperature") %>%
    group_by(encntr_id, event) %>%
    summarize_at("result_val", max, na.rm = TRUE) %>%
    pivot_wider(
        names_from = event, 
        values_from = result_val,
        names_prefix = "max_"
    )
```

```{r sum_values}
sum_vals <- data_spark %>%
    filter(event == "uop") %>%
    group_by(encntr_id, event) %>%
    summarize_at("result_val", sum, na.rm = TRUE) %>%
    pivot_wider(
        names_from = event, 
        values_from = result_val,
        names_prefix = "sum_"
    )
```

```{r drips}
drips <- data_drips %>%
    mutate(drip = paste(str_extract(medication, "^.{3}"), rate)) %>%
    group_by(encntr_id) %>%
    summarize_at("drip", str_c, collapse = ", ")

```


```{r datatable}
col_names <- c(
    "Room",
    "Patient", 
    "Age",
    "Sex",
    "Wt",
    "LOS",
    "SOFA",
    "Temp Trend",
    "Tmax",
    "SCr Trend",
    "SCr",
    "CrCl",
    "WBC Trend", 
    "WBC",
    "B%",
    "Hgb Trend",
    "Hgb",
    "Plt Trend",
    "Plt",
    "SBP Trend",
    "HR Trend", 
    "Gluc Trend",
    # "SSI", 
    "INR Trend",
    "INR",
    "PTT",
    "UOP Trend",
    "UOP",
    "Avg",
    "Net",
    "Dialysis",
    "Drips",
    "Devices",
    "Sod",
    "Pot",
    "T.Bili",
    "AST",
    "ALT",
    # "Alerts",
    "A1c",
    "TSH",
    "Chol",
    "Trig",
    "HDL",
    "LDL",
    "PCA",
    "Doses",
    "MRSA"
)

df <- data_dash %>% 
    left_join(sparks, by = "encntr_id") %>%
    left_join(curr_vals, by = "encntr_id") %>%
    left_join(max_vals, by = "encntr_id") %>%
    left_join(sum_vals, by = "encntr_id") %>%
    left_join(drips, by = "encntr_id") %>%
    select(
        bed,
        name,
        age,
        sex,
        weight,
        los,
        sofa,
        temperature,
        max_temperature,
        scr,
        curr_scr,
        crcl,
        wbc,
        curr_wbc,
        bands,
        hgb,
        curr_hgb,
        platelet,
        curr_platelet,
        sbp,
        hr,
        glucose,
        inr,
        curr_inr,
        ptt,
        uop,
        sum_uop,
        uop_avg,
        net_io,
        dialysis,
        drip,
        iabp,
        sodium,
        potassium,
        tbili,
        ast,
        alt,
        a1c,
        tsh,
        chol,
        trig,
        hdl,
        ldl,
        pca_doses,
        pca_demands,
        mrsa_pcr
    )

df %>%
    datatable(
        # caption = paste0(
        #     "Data updated: ", 
        #     format(file.info(file_nm)$mtime, "%B %d, %Y at %I:%M %p")
        # ),
        caption = htmltools::tags$caption(
            style = "caption-side: bottom",
            paste0(
                "Data updated: ", 
                format(file.info(file_nm)$mtime, "%B %d, %Y at %I:%M %p")
            )
        ),
        rownames = FALSE, 
        escape = FALSE,
        colnames = col_names,
        # extensions = "FixedHeader",
        extensions = c("FixedColumns", "FixedHeader"),
        options = list(
            dom = "t",
            # fixedColumns = TRUE,
            fixedColumns = list(leftColumns = 2),
            fixedHeader = TRUE,
            scrollX = TRUE,
            pageLength = 21,
            order = list(list(0, 'asc')),
            initComplete = JS(
                "function(settings, json) {",
                "$(this.api().table().header()).css({'font-size': '80%'});",
                "}"
            ),
            fnDrawCallback = JS(
                "function(){ HTMLWidgets.staticRender(); }"
            )
        )
    ) %>%
    formatRound(c("tsh", "curr_inr", "curr_scr"), 2) %>%
    formatRound(c("los", "max_temperature", "uop_avg", "a1c", "potassium", "curr_wbc", "curr_hgb"), 1) %>%
    formatRound(c("crcl", "sum_uop", "net_io", "ptt", "weight"), 0) %>%
    formatStyle(columns = c(1:ncol(df)), fontSize = "80%") %>%
    spk_add_deps()



```

