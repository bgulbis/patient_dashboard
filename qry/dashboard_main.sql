WITH CURR_PTS AS (
	...
	WHERE
		...
		AND CLINICAL_EVENT.EVENT_END_DT_TM BETWEEN 
			pi_to_gmt(TRUNC(SYSDATE - 1) + 7/24, 'America/Chicago')
			AND pi_to_gmt(TRUNC(SYSDATE) + 7/24 - 1/86400, 'America/Chicago')
		AND CLINICAL_EVENT.VALID_UNTIL_DT_TM > DATE '2099-12-31'
	...
	WHERE
		...
		AND CLINICAL_EVENT.EVENT_END_DT_TM >= pi_to_gmt(SYSDATE - 2, 'America/Chicago')
	GROUP BY
		CLINICAL_EVENT.ENCNTR_ID
), IABP AS (
	SELECT DISTINCT
...
SELECT DISTINCT
    ...
    SYSDATE - pi_from_gmt(CURR_PTS.REG_DT_TM, 'America/Chicago') AS LOS, 
    CURR_PTS.AGE AS AGE,
	...
	SOFA_UOP.UOP / WEIGHT.RESULT_VAL / (TO_NUMBER(SYSDATE - pi_from_gmt(SOFA_UOP.FIRST_UOP_DATETIME, 'America/Chicago')) * 24) AS UOP_AVG,
	DIALYSIS.DIALYSIS,
	IABP.IABP,
	...
	CASE
		WHEN LABS_COVID.RESULT_VAL = 'Not Detected' THEN 'Neg'
		ELSE SUBSTR(LABS_COVID.RESULT_VAL, 1, 3)
	END AS COVID_19
FROM
	CURR_PTS,
	DIALYSIS,
...
WHERE
	...
	AND CURR_PTS.ENCNTR_ID = LABS_COVID.ENCNTR_ID(+)
