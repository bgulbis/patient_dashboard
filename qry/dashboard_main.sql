WITH CURR_PTS AS (
	...
	WHERE
		...
		AND CLINICAL_EVENT.EVENT_END_DT_TM BETWEEN 
			pi_to_gmt(TRUNC(SYSDATE - 1) + 7/24, 'CST')
			AND pi_to_gmt(TRUNC(SYSDATE) + 7/24 - 1/86400, 'CST')
		AND CLINICAL_EVENT.VALID_UNTIL_DT_TM > DATE '2099-12-31'
	...
	WHERE
		...
		AND CLINICAL_EVENT.EVENT_END_DT_TM >= pi_to_gmt(SYSDATE - 2, 'CST')
	GROUP BY
		CLINICAL_EVENT.ENCNTR_ID
), IABP AS (
	SELECT DISTINCT
...
SELECT DISTINCT
    ...
    SYSDATE - pi_from_gmt(CURR_PTS.REG_DT_TM, 'CST') AS LOS, 
    CURR_PTS.AGE AS AGE,
	...
	SOFA_UOP.UOP / WEIGHT.RESULT_VAL / (TO_NUMBER(SYSDATE - pi_from_gmt(SOFA_UOP.FIRST_UOP_DATETIME, 'CST')) * 24) AS UOP_AVG,
	DIALYSIS.DIALYSIS,
	IABP.IABP,
